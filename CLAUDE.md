...
| `hooks/` | Git hooks shared by **every repo on the machine** via `core.hooksPath` (set globally in `.config/git/config`, so it propagates with the dotfiles). One real script — `_chain` — with every hook name symlinked to it; `$0`'s basename selects the behavior. Each hook runs the repo's **own** `.git/hooks/<name>` first, then any lnk-side guard, because `core.hooksPath` **replaces** the hooks directory wholesale and would otherwise silently disable per-repo hooks (husky, pre-commit framework) everywhere. Currently guards: `pre-commit` blocks an `autoMode` key in a staged `.claude/settings.json` |
